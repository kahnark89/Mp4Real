...
- **`failure_mode_tag` vocabulary is append-only; refinements create a new tag with a `subsumes` link, never a rename.** [Q3.2, 2026-05-26 — selected under author "no preference," overridable] Old events keep their original tag and stay valid forever; a refined concept gets a *new* tag declaring `subsumes: [old_tag]`, so queries walk the subsumption chain without mutating historical records. Chosen to preserve corpus immutability (a tag is provenance — never rewrite a past one) and satisfy the append-only-history invariant. Rejected: aliasing tables and per-release ontology versioning — both push interpretation to query time and risk silently remapping historical meaning.
...
- **Codebook is append-only; tidiness comes from archiving, never deletion.** [Q4.2, 2026-05-26] Once a primitive enters the book it is never removed — its occurrence is a preserved fact. A primitive that goes cold (no triggers over a window, or an outlier) is demoted to an `archived` / `legacy_primitives` set: excluded from active-session matching to keep live matching tidy, but retained and auto-re-promoted to active if a matching event recurs. The fact of occurrence outlives the primitive's active use.
- **Twin update cadence is novelty-driven, not scheduled — a continuous learn↔serve loop.** [Q4.3, 2026-05-26] After the first LoRA at ~200 events, the adapter refreshes when statistically novel data accumulates (drift / novelty signal), not on a fixed clock and not on raw event count alone. Target: a fluid back-and-forth — novel events feed learning, the refreshed Twin emits statistically relevant guidance and predictions, and divergence from the corpus distribution schedules the next refresh. Mirrors the MDL/codec identity — novelty is the bits that cost, so novelty is the signal to learn.
...
- **Provider is chosen per deployment, uniformly across tasks — not routed per use case.** [Q5.1, 2026-05-26] A facility's config picks the LLM provider; the same provider serves both PIE elicitation (structured output) and Twin guidance (synthesis). Code stays provider-agnostic behind `LlmClient`; the elicitation-vs-guidance distinction is handled by prompt/parse contracts, not by binding different providers to different tasks. Keeps deployments simple and avoids coupling task code to a provider's particular strengths.
