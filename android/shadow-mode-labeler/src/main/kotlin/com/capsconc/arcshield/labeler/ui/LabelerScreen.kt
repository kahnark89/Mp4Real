 ...
 *
 * Shows either the shift picker (no log selected) or the labeling pane
 * (log selected). Single-screen: no nav library dependency.
 *
 * Wire into an Activity or NavHost as:
 *   LabelerScreen()
 */
@Composable
fun LabelerScreen(vm: LabelerViewModel = viewModel()) {
...
private fun ShiftPickerPane(vm: LabelerViewModel) {
    ...
    Scaffold(
        topBar = { TopAppBar(title = { Text("Shadow mode logs") }) }
    ) { padding ->
        if (logs.isEmpty()) {
            Box(
